...
void server_node_thread(void *arg);
...
int checkArray(json_object *jarray, int * sock, struct sockaddr_in *client);
int checkList(int64_t jfrom, int * sock, struct sockaddr_in *client);
int send_message(int64_t dest, char * message, int * sock, struct sockaddr_in *client, int64_t type);

int SocketCreate(int port)
{
	printf("Port in socket create %d \n", port);
	int socket_desc;
    ...
    int iRetval=-1;
	int ClientPort = port; // Multiple clients using diffrent ports.
...
int checkList(int64_t jfrom, int * sock, struct sockaddr_in * client){
	...
	//char * regMesg = 
	send_setup(jfrom,5, NULL, sock, client);
	send_message(jfrom, "Registration (^)", sock, client, 9); // Send Registration message.
    //printf("%" PRId64", %d\n", sub[sizeSub],sizeSub);
    sizeSub++;
    return -1;
}
...
int send_setup(int64_t jfrom, int type,  json_object *jarray, int *sock, struct sockaddr_in *client){
	if(type == 6){
		checkList(jfrom, sock, client);
		checkArray(jarray, sock, client);
    ...
    for(int i = 0; i < json_object_array_length(jarray); i++){
        ...
        switch (jtype){
            case json_type_int:// NodeId
                checkList(json_object_get_int(jobj), sock, client);
                break;
            case json_type_array:
                temp = json_object_get(jobj);
                if(json_object_array_length(temp) != 0){
                    checkArray(temp, sock, client);
                }
                break;
        }
    }
...
int send_broadcast(char * message, int * sock, struct sockaddr_in * client){
    ...
    send_message(0, message, sock, client, 8);
    return 1;
}
...
void server_node_thread(void *arg){	
	...
	printf("IN START FOR PORT # %d \n", portNum);
	int socket_desc , sock , clientLen , read_size;
    char client_message[1000]={0};
    int recv_Size= 150;
    socket_desc = SocketCreate(portNum);
    ...
    for(int i = 0; i < 20; i++){
        ...
        if(recive_message(&client_message, &sock,&recv_Size) < 0){
			printf(" test error thread recive \n");
...
int main(int argc , char *argv[])
{
	...
	//strcat(portNum, '\0');
	printf("portNum = %s\n", portNum);
	printf("about to atoi\n");
	int port = atoi(portNum);
	printf("int form of port %d\n", port);
	server_node_thread(&port);
}